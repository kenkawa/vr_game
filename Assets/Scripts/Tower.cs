...
///   ・プレイヤーは、塔の頂上の丸い足場(半径 1.6m)の真ん中に立つ。足場のまわりは、細い手すりだけ(下が見える)
///   ・塔のまわりは、全方向(360 度)から敵が来る。地上の敵は、塔をぐるりと囲む外柵(木の柵の輪)まで来て、柵と塔を攻撃する。
///     空の敵は、頂上のまわりを飛び回って襲ってくる
///   ・耐久値が 0 になると、ゲームオーバー
///
/// 外柵を塔の足元ではなく、遠く(既定 15.5m)の輪にしている理由:
///   頂上(高さ 12m)の真ん中に立つと、足場のふちに隠れて、塔から約 13m 以内の地面は見えない
///   (足場の半径 1.6m × 目の高さ ÷ 足場のふちまでの高さ差)。そこで、柵の輪を、ちょうど見えはじめる距離の外に置いて、
...
public class Tower : MonoBehaviour, IStronghold
{
    ...
    public const float PlatformRadius = 1.6f;
    ...
    public void Build(float maxHealthValue, float heightMeters, float fenceRadiusMeters)
    {
        ...
        var iron = new MeshKit.Batch();     // 鉄(矢を撃つ細い窓、手すり、門の帯、たいまつの受け)
        ...
        Solid("Ground", PrimitiveType.Cube, Axis(ground - 0.5f), new Vector3(900f, 1f, 900f),
            GroundTint, ProcTex.Grass(), 150f, 150f, true, false);
        Disc("DirtRing", Axis(ground + 0.02f), fenceRadius + 6f, 0.04f, DirtTint, ProcTex.Dirt(), (fenceRadius + 6f) * 2f / tile);
        ...
        for (int i = 0; i < SegmentHeights.Length; i++)
        {
            ...
            {
                float beltDiameter = SegmentRadii[i] + SegmentRadii[i + 1] + 0.3f;
        ...
        // 足元の模様:金の輪と、赤い円(絨毯)
        Disc("CarpetGold", Axis(top + 0.006f), 1.42f, 0.012f, GoldColor, null, 1f);
        Disc("CarpetRed", Axis(top + 0.012f), 1.26f, 0.012f, RedColor, null, 1f);

        // 手すり:細い柱と、2 本の横棒だけ(下の敵が見えるように、ふさがない)
        const int railPosts = 16;
        float railRadius = PlatformRadius - 0.08f;
        for (int i = 0; i < railPosts; i++)
        {
            float a = i * 360f / railPosts;
            Vector3 p = Axis(top) + Direction(a) * railRadius;
            iron.Add(cyl6, p, Quaternion.identity, new Vector3(0.05f, 0.98f, 0.05f));

            // 隣の柱までの横棒(円周を、直線で近づける)
            Vector3 q = Axis(top) + Direction(a + 360f / railPosts) * railRadius;
            Vector3 along = q - p;
            Quaternion rot = Quaternion.LookRotation(along.normalized);
            iron.Add(cube, (p + q) * 0.5f + Vector3.up * 0.95f, rot, new Vector3(0.05f, 0.04f, along.magnitude + 0.02f));
            iron.Add(cube, (p + q) * 0.5f + Vector3.up * 0.5f, rot, new Vector3(0.035f, 0.03f, along.magnitude + 0.02f));
        }

        // 旗ざお 4 本(ななめ前後の 4 か所)。赤い旗が風で揺れる
        foreach (float a in new[] { 45f, 135f, 225f, 315f })
        {
            Vector3 p = Axis(top) + Direction(a) * railRadius;
            wood.Add(cyl6, p, Quaternion.identity, new Vector3(0.07f, 2.6f, 0.07f));
            BuildFlag(p + Vector3.up * 2.45f, Quaternion.LookRotation(Direction(a + 90f)));
        }

        // かがり火 2 つ(後ろの左右)。光る炎だけ
        foreach (float a in new[] { 150f, 210f })
        {
            Vector3 p = Axis(top) + Direction(a) * (PlatformRadius - 0.3f);
            wood.Add(cyl8, p, Quaternion.identity, new Vector3(0.1f, 0.85f, 0.1f));
            iron.Add(cyl8, p + Vector3.up * 0.84f, Quaternion.identity, new Vector3(0.36f, 0.07f, 0.36f));
            FlameFlicker.Create(transform, p + Vector3.up * 0.93f, 0.9f, 0f, 3);
        }
        ...
        float gateR = SegmentRadii[0] + 0.02f;
        ...
        foreach (float a in new[] { 45f, 135f, 225f, 315f })
        {
            BuildBanner(Axis(ground + SegmentHeights[0] + SegmentHeights[1] - 0.3f) + Direction(a) * (SegmentRadii[1] + 0.06f),
        ...
        {
            float a = i * 45f + 22.5f;
            iron.Add(cube, Axis(slitY) + Direction(a) * (SegmentRadii[2] + 0.01f), Quaternion.LookRotation(Direction(a)), new Vector3(0.12f, 0.7f, 0.06f));
        }

        // 外柵(木の柵の輪)。とがった杭を、ぐるりと並べて、横木で結ぶ。地上の敵は、この柵の外側まで来て、柵と塔を攻撃する
        float circumference = 2f * Mathf.PI * fenceRadius;
        int postCount = Mathf.CeilToInt(circumference / 0.55f);
        for (int i = 0; i < postCount; i++)
        {
            float a = i * 360f / postCount;
            Vector3 p = Axis(ground) + Direction(a) * fenceRadius;
            float postHeight = 1.5f + 0.4f * Mathf.Abs(Mathf.Sin(i * 12.9898f));
            posts.Add(cyl6, p, Quaternion.identity, new Vector3(0.3f, postHeight, 0.3f));
            posts.Add(cone6, p + Vector3.up * postHeight, Quaternion.identity, new Vector3(0.3f, 0.35f, 0.3f));
        }
        ...
        for (int i = 0; i < beamCount; i++)
        {
            ...
            Vector3 p = Axis(ground) + Direction(a0) * (fenceRadius - 0.17f);
            Vector3 q = Axis(ground) + Direction(a1) * (fenceRadius - 0.17f);
            Vector3 along = q - p;
            Quaternion rot = Quaternion.LookRotation(along.normalized);
            beams.Add(cube, (p + q) * 0.5f + Vector3.up * 0.5f, rot, new Vector3(0.1f, 0.12f, along.magnitude + 0.05f));
            beams.Add(cube, (p + q) * 0.5f + Vector3.up * 1.1f, rot, new Vector3(0.1f, 0.12f, along.magnitude + 0.05f));
        }

        // 柵の内側に、かがり火(8 か所)
        for (int i = 0; i < 8; i++)
        {
            Vector3 p = Axis(ground) + Direction(i * 45f + 22.5f) * (fenceRadius - 0.9f);
            wood.Add(cyl8, p, Quaternion.identity, new Vector3(0.14f, 2f, 0.14f));
            iron.Add(cyl8, p + Vector3.up * 2f, Quaternion.identity, new Vector3(0.34f, 0.08f, 0.34f));
            FlameFlicker.Create(transform, p + Vector3.up * 2.1f, 1.1f, 0f, 2);
        }
    ...
    void BuildFlag(Vector3 position, Quaternion rotation)
    {
